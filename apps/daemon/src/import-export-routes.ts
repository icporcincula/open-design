...
export function registerImportRoutes(app: Express, ctx: RegisterImportRoutesDeps) {
  ...
  // the same trust-gate + realpath + sandbox checks as `POST
  // /api/import/folder`, but updates `metadata.baseDir` on an existing
  // project record rather than creating a new one. Use case: the user
  // wants the project to live in a different folder going forward
  // (image 3 of the design brief — 清空并替换目录). Subsequent reads
  // and writes route through the new directory; no files are moved by
  // the daemon — the user is responsible for relocating content
  // they want to keep, same as paseo's "move workspace" gesture.
  app.post('/api/projects/:id/working-dir', async (req, res) => {
    try {
      const projectId = req.params.id;
      const existing = ctx.projectStore.getProject(db, projectId);
      if (!existing) {
      ...
      // Stamp the existing project record. We re-detect entryFile in the
      // new directory so the chat agent has a starting point; if the
      // directory is empty, entryFile stays null and the agent treats
      // this as a fresh workspace.
      ...
      const updated = ctx.projectStore.updateProject(db, projectId, {
        metadata: nextMeta,
      });
      if (!updated) {
        return sendApiError(res, 404, 'PROJECT_NOT_FOUND', 'project not found');
      ...
      res.json({ project: updated, baseDir: normalizedPath, entryFile });
    } catch (err: any) {
